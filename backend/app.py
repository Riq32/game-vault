...
def calculate_gamification_state(total_xp):
    ...
    return {
        "level": level, "total_xp": total_xp, "xp_into_level": xp_into_level, 
        "xp_needed": xp_needed_for_next, "progress_percentage": progress_percentage
    }

def generate_smart_notifications(user):
    # Leaderboard Movement Check
...
def modify_vault_item(item_id):
    ...
    if request.method == 'PATCH':
        ...
        if new_status != item.status:
            if new_status == 'Playing': db.session.add(Notification(user_id=user.id, title="Status Updated", message=f"Now actively playing '{item.game_name}'.", type="activity"))
            elif new_status == 'Backlog': db.session.add(Notification(user_id=user.id, title="Status Updated", message=f"Moved '{item.game_name}' back to Backlog.", type="activity"))
        ...
        if level_up: db.session.add(Notification(user_id=user.id, title="Level Up!", message=f"Congratulations! You've reached Level {new_level}.", type="achievement"))
        db.session.commit()
