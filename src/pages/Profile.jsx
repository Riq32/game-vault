...
export default function Profile() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setSaveMessage({ 
        type: 'error', 
        text: err.response?.data?.error || 'Failed to update identity parameters.' 
      });
    } finally {
      setIsSaving(false);
    }
  ...
  if (error) return (
    <div className="min-h-screen bg-[var(--color-vault-black)] pt-32 flex flex-col items-center justify-center px-6">
      <div className="bg-red-500/10 border border-red-500/50 p-6 rounded-2xl flex flex-col items-center text-center max-w-md">
        ...
        <button onClick={() => navigate('/auth')} className="bg-red-500 text-black px-6 py-2 rounded-full font-black uppercase tracking-wider text-sm">
          Return to Login
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[var(--color-vault-black)] pt-32 pb-20 px-6">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <motion.div 
          ...
          className="bg-[var(--color-vault-surface)] border border-[var(--color-vault-border)] rounded-3xl p-8 md:p-12 relative overflow-hidden transition-all"
        >
            ...
            <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
              ...
              <div className="flex-grow text-center md:text-left">
                ...
                <p className="text-[var(--color-text-secondary)] font-medium mb-2 flex items-center justify-center md:justify-start gap-2">
                  ...
                  {profileData.email && <span className="opacity-50">|</span>}
                  {profileData.email && <span className="text-sm">{profileData.email}</span>}
                </p>
                <p className="text-[var(--color-text-secondary)] font-bold uppercase tracking-widest text-xs mb-6 text-[var(--color-neon-cyan)]">
              ...
              <div className="flex gap-3 mt-6 md:mt-0">
                <button onClick={() => setIsEditing(true)} className="bg-[var(--color-vault-black)] border border-[var(--color-vault-border)] hover:border-[var(--color-neon-cyan)] text-[var(--color-text-secondary)] hover:text-[var(--color-neon-cyan)] p-3 rounded-xl transition-colors flex items-center gap-2">
                  <Edit2 size={18} /> <span className="text-sm font-bold uppercase hidden md:block">Edit Profile</span>
                </button>
                <button onClick={handleLogout} className="bg-[var(--color-vault-black)] border border-red-500/50 hover:border-red-500 text-red-500/80 hover:text-red-500 p-3 rounded-xl transition-colors">
                  <LogOut size={18} />
                </button>
              </div>
            </div>
            ...
            >
              <div className="flex justify-between items-center mb-8 border-b border-[var(--color-vault-border)] pb-4">
                <h2 className="text-2xl font-black uppercase tracking-tighter flex items-center gap-2">
                  <Settings className="text-[var(--color-neon-cyan)]" /> Configure Identity
                </h2>
                <button type="button" onClick={() => { setIsEditing(false); setSaveMessage({type:'', text:''}); }} className="text-[var(--color-text-secondary)] hover:text-white transition-colors">
                  <X size={24} />
                </button>
              </div>
                  ...
                  <motion.div 
                    ...
                    className={`mb-6 p-4 rounded-xl flex items-center gap-3 text-sm font-bold border ${
                      saveMessage.type === 'error' 
                        ...
                        : 'bg-[var(--color-neon-cyan)]/10 border-[var(--color-neon-cyan)]/50 text-[var(--color-neon-cyan)]'
                    }`}
                  >
                    {saveMessage.type === 'error' ? <AlertCircle size={18} /> : <CheckCircle2 size={18} />}
                    {saveMessage.text}
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <User className="text-[var(--color-text-secondary)] group-focus-within:text-[var(--color-neon-cyan)] transition-colors" size={18} />
                      </div>
                      <input type="text" value={formData.displayName} onChange={(e) => setFormData({...formData, displayName: e.target.value})} placeholder="How should we call you?" className="w-full bg-[var(--color-vault-black)] border border-[var(--color-vault-border)] text-white rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[var(--color-neon-cyan)] transition-colors" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <Mail className="text-[var(--color-text-secondary)] group-focus-within:text-[var(--color-neon-cyan)] transition-colors" size={18} />
                      </div>
                      <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="hunter@vault.net" className="w-full bg-[var(--color-vault-black)] border border-[var(--color-vault-border)] text-white rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[var(--color-neon-cyan)] transition-colors" />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative group mb-4">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <ImageIcon className="text-[var(--color-text-secondary)] group-focus-within:text-[var(--color-neon-cyan)] transition-colors" size={18} />
                    </div>
                    <input type="url" value={formData.profilePic} onChange={(e) => setFormData({...formData, profilePic: e.target.value})} placeholder="https://example.com/avatar.jpg" className="w-full bg-[var(--color-vault-black)] border border-[var(--color-vault-border)] text-white rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[var(--color-neon-cyan)] transition-colors" />
                  </div>
                  ...
                  <div className="flex items-center gap-4 p-4 bg-[var(--color-vault-black)] border border-[var(--color-vault-border)] rounded-xl">
                    <div className="w-16 h-16 rounded-full border border-[var(--color-vault-border)] overflow-hidden bg-[var(--color-vault-surface)] flex items-center justify-center">
                      {formData.profilePic ? (
                        <img src={formData.profilePic} alt="Preview" className="w-full h-full object-cover" onError={(e) => { e.target.style.display = 'none'; }} />
                      ) : (
                        <User size={24} className="text-[var(--color-text-secondary)]" />
                      )}
                    </div>
                    <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">Ensure the image link ends in .jpg, .png, or .gif for best compatibility.</p>
                  </div>
                </div>
              </div>

              <div className="flex justify-end gap-4 border-t border-[var(--color-vault-border)] pt-6">
                <button type="button" onClick={() => setIsEditing(false)} disabled={isSaving} className="px-6 py-3 rounded-full font-bold uppercase tracking-widest text-sm text-[var(--color-text-secondary)] hover:text-white transition-colors disabled:opacity-50">
                  Cancel
                </button>
                <button type="submit" disabled={isSaving} className="flex items-center gap-2 bg-[var(--color-neon-cyan)] text-black px-8 py-3 rounded-full font-black uppercase tracking-widest text-sm hover:scale-105 transition-transform disabled:opacity-50 shadow-[0_0_15px_rgba(0,240,255,0.2)]">
